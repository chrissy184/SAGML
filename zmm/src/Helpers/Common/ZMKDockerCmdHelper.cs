...
namespace ZMM.Helpers.Common
{
    ...
    {       
        public static Dictionary<string,string> StartCommands = new Dictionary<string, string>()
            {
                {"zmk1", "sudo docker run -d --net=global-zmod-net --ip=\"172.20.0.4\" -p 8000:8000 --name=zmk1 --volumes-from zmm store/softwareag/mlw-zmk:10.5"},
                {"zmk2", "sudo docker run -d --net=global-zmod-net --ip=\"172.20.0.5\" -p 8001:8000 --name=zmk2 --volumes-from zmm store/softwareag/mlw-zmk:10.5"},
                {"zmk3", "sudo docker run -d --net=global-zmod-net --ip=\"172.20.0.6\" -p 8002:8000 --name=zmk3 --volumes-from zmm store/softwareag/mlw-zmk:10.5"}
            }; 
